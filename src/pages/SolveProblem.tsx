...
// Flatten a CauseNode tree into a flat list (all descendants).
function flattenCauseTree(nodes: CauseNode[]): CauseNode[] {
  return nodes.flatMap(n => [n, ...flattenCauseTree(n.children)])
}

// Recursively update solutions on a matching node.
function updateSolutions(causes: CauseNode[], nodeId: string, solutions: string[]): CauseNode[] {
  return causes.map(c => ({
    ...c,
...
export default function SolveProblem({ id, onDone }: Props) {
  ...
  // Local state: solutions per cause id (pre-filled from saved data)
  const [solutionsMap, setSolutionsMap] = useState<Map<string, string[]>>(() => {
  ...
  function saveSolutions() {
    ...
    solutionsMap.forEach((solutions, causeId) => {
      causes = updateSolutions(causes, causeId, solutions)
    })
              ...
              return (
                <div key={rc.id}>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <span className="inline-flex items-center px-3 py-1 bg-amber-100 text-amber-800 text-sm font-semibold rounded-full">
                      Root Cause
                    </span>
                    ...
                    <ul className="space-y-1.5 mb-3">
                      {sols.map((sol, i) => (
                        <li key={i} className="flex items-start gap-2 bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2">
                          <span className="flex-1 text-sm text-indigo-900">{sol}</span>
                          <button
                            onClick={() => removeSolution(rc.id, i)}
                            className="text-indigo-300 hover:text-indigo-600 text-xs mt-0.5 transition-colors shrink-0"
                            title="Remove"
                          >
                            ✕
                          </button>
